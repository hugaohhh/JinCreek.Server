...
namespace JinCreek.Server.AdminTests.SimAndDevicesTests
{
    ...
    public class UpdateMineTests : IClassFixture<CustomWebApplicationFactory<JinCreek.Server.Admin.Startup>>
    {
        ...
        {
            var obj = new { };
            var (response, body, _) = Utils.Put(_client, $"{Url}/", Utils.CreateJsonContent(obj), "user1", "user1", 1, "domain01"); // ユーザー管理者
        ...
        public void Case02()
        {
            ...
            var (response, _, json) = Utils.Put(_client, $"{Url}/aa", Utils.CreateJsonContent(obj), "user1", "user1", 1, "domain01"); // ユーザー管理者
        ...
        {
            var obj = new { };
            var (response, _, json) = Utils.Put(_client, $"{Url}/{_simDevice1.Id}", Utils.CreateJsonContent(obj), "user1", "user1", 1, "domain01"); // ユーザー管理者
            Assert.Equal(HttpStatusCode.BadRequest, response.StatusCode);
            Assert.NotNull(json["traceId"]); // 存在
        ...
        public void Case06()
        {
            ...
            var (response, _, json) = Utils.Put(_client, $"{Url}/{Guid.NewGuid()}", Utils.CreateJsonContent(obj), "user1", "user1", 1, "domain01"); // ユーザー管理者
