...
namespace JinCreek.Server.AdminTests.SimTests
{
    ...
    public class UpdateMineTests : IClassFixture<CustomWebApplicationFactoryWithMariaDb<Admin.Startup>>
    {
        ...
        public void Case03()
        {
            ...
            Assert.Equal(HttpStatusCode.BadRequest, result.StatusCode);
            Assert.NotNull(json["traceId"]); // 存在
            Assert.Equal("The SimGroupId field is required.", json["errors"]?["SimGroupId"].First.ToString());
        ...
        public void Case04A()
        {
            ...
            var obj = new
            {
                SimGroupId = "13135465464", //UUID　ではない
                Msisdn = "1001",
                Imsi = "1001",
                Iccid = "1001",
                UserName = "user01",
        ...
        public void Case06()
        {
            ...
            var result = Utils.Put(_client, $"{Url}/{_sim2.Id}", Utils.CreateJsonContent(obj), token);
            var body = result.Content.ReadAsStringAsync().Result;
            var json = JObject.Parse(body);
            Assert.Equal(HttpStatusCode.Forbidden, result.StatusCode);
            Assert.NotNull(json["traceId"]); // 存在
