...
namespace JinCreek.Server.AdminTests.SimTests
{
    ...
    public class ListingTests : IClassFixture<CustomWebApplicationFactory<JinCreek.Server.Admin.Startup>>
    {
        ...
        {
            var token = Utils.GetAccessToken(_client, "user0", "user0"); //    スーパー管理者
            var result = Utils.Get(_client, $"{Url}/?OrganizationCode=abcde1234", token);
            var body = result.Content.ReadAsStringAsync().Result;
            Assert.Equal(HttpStatusCode.BadRequest, result.StatusCode);
            var json = JObject.Parse(body);
            Assert.NotNull(json["traceId"]);
        ...
        {
            var token = Utils.GetAccessToken(_client, "user0", "user0"); //    スーパー管理者
            var result = Utils.Get(_client, $"{Url}/?OrganizationCode=13465", token);
            var body = result.Content.ReadAsStringAsync().Result;
        ...
        public void Case08()
        {
            ...
            _context.SaveChanges();
            var token = Utils.GetAccessToken(_client, "user0", "user0"); //    スーパー管理者
            var result = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=1&SimGroupId={_simGroup1.Id}&sortBy=SimGroupName&orderBy=asc", token);
            var body = result.Content.ReadAsStringAsync().Result;
            var json = JObject.Parse(body);
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, result.StatusCode);
        ...
        public void Case09()
        {
            ...
            _context.SaveChanges();
            var token = Utils.GetAccessToken(_client, "user0", "user0"); //    スーパー管理者
            var result = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=2&pageSize=2&Msisdn=msisdn&sortBy=SimGroupName&orderBy=desc", token);
            var body = result.Content.ReadAsStringAsync().Result;
            var json = JObject.Parse(body);
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, result.StatusCode);
        ...
        public void Case10()
        {
            ...
            _context.SaveChanges();
            var token = Utils.GetAccessToken(_client, "user0", "user0"); //    スーパー管理者
            var result = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=3&pageSize=3&UserName=sim&sortBy=Msisdn&orderBy=asc", token);
            var body = result.Content.ReadAsStringAsync().Result;
            var json = JObject.Parse(body);
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, result.StatusCode);
        ...
        public void Case11()
        {
            ...
            _context.SaveChanges();
            var token = Utils.GetAccessToken(_client, "user0", "user0"); //    スーパー管理者
            var result = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=2&Msisdn=msisdn&UserName=sim&SimGroupId={_simGroup1.Id}&sortBy=Msisdn&orderBy=desc", token);
            var body = result.Content.ReadAsStringAsync().Result;
            var json = JObject.Parse(body);
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, result.StatusCode);
        ...
        public void Case12()
        {
            ...
            _context.SaveChanges();
            var token = Utils.GetAccessToken(_client, "user0", "user0"); //    スーパー管理者
            var result = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=1&pageSize=3&Msisdn=msisdn&UserName=sim&SimGroupId={_simGroup1.Id}&sortBy=UserName&orderBy=asc", token);
            var body = result.Content.ReadAsStringAsync().Result;
            var json = JObject.Parse(body);
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, result.StatusCode);
        ...
        public void Case14()
        {
            ...
            _context.SaveChanges();
            var token = Utils.GetAccessToken(_client, "user0", "user0"); //    スーパー管理者
            var result = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=1&Msisdn=msisdn&UserName=sim&SimGroupId={_simGroup1.Id}", token);
            var body = result.Content.ReadAsStringAsync().Result;
            var json = JObject.Parse(body);
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, result.StatusCode);
