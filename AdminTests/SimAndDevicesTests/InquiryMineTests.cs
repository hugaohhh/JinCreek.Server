...
namespace JinCreek.Server.AdminTests.SimAndDevicesTests
{
    ...
    public class InquiryMineTests : IClassFixture<CustomWebApplicationFactory<JinCreek.Server.Admin.Startup>>
    {
        ...
        public void Case01()
        {
            var (response, _, json) = Utils.Get(_client, $"{Url}/abcde1234", "user1", "user1", 1, "domain01"); // ユーザー管理者
        ...
        public void Case02()
        {
            var (response, _, json) = Utils.Get(_client, $"{Url}/{Guid.NewGuid()}", "user1", "user1", 1, "domain01"); // ユーザー管理者
        ...
        /// <summary>
        /// DBにSIM&端末組合せ：存在 組織 : '自組織　利用終了日：不在
        /// </summary>
        [Fact]
        public void Case05()
        {
