...
namespace JinCreek.Server.AdminTests.DashboardTests
{
    ...
    public class InquiryMineTests : IClassFixture<CustomWebApplicationFactoryWithMariaDb<JinCreek.Server.Admin.Startup>>
    {
        ...
        private const string Url = "/api/dashboard/mine";
        ...
        public void Case01()
        {
            var (response, _, json) = Utils.Get(_client, Url, "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Equal("8", json["onlineUsers"]); //自組織のonlineUsers
            Assert.Equal("3", json["totalUsers"]);　//自組織のUser
            Assert.Equal("2", json["totalDevices"]);　// 自組織の端末
            Assert.Equal("2", json["windowsDevices"]);
