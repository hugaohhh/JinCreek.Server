...
namespace JinCreek.Server.Auth.Controllers
{
    ...
    public class SimDeviceAuthenticationController : ControllerBase
    {
        ...
        public IActionResult Authentication(SimDeviceAuthenticationRequest simDeviceAuthenticationRequest)
        {
            ...
            var device = simDevice.Device;
            AdDevice adDevice = _authenticationRepository.QueryAdDevice(device.Id);
            
            // 認証成功のSimDeviceによって　それに対応する LoginできるUser を検索します
            var canLogonUsers = _authenticationRepository.QueryLoginUsers(simDevice);

            _radiusDbRepository.SimDeviceAuthUpdateRadreply(simDevice.Sim, true);
            ...
            else if (simDeviceAuthenticationStateDone != null)
            {
