...
namespace JinCreek.Server.AdminTests.DeviceTests
{
    ...
    public class CsvImportMineTests : IClassFixture<CustomWebApplicationFactoryWithMariaDb<JinCreek.Server.Admin.Startup>>
    {
        ...
        public void Case07()
        {
            ...
            var obj = new StringBuilder();
            obj.AppendLine(CsvHeader);
            obj.AppendLine($"{device2.Id},domain1,deviceGroup1,device2,11,1001,type01,a,,{_organizationClientApp1.Id},,,False,0,0001-01-01,");
            obj.AppendLine($"{device1.Id},domain1,deviceGroup1,device1,1,1001,type01,{_lte1.Id},lte1,{_organizationClientApp1.Id},,,False,0,0001-01-01,");
        ...
        /// <summary>
        /// 全部正常（端末レコード:他組織）　端末レコード：'存在（自組織）Windowsサインイン許可リストキャッシュ日数:0
        /// </summary>
        [Fact]
        public void Case13()
        {
            ...
            var obj = new StringBuilder();
            obj.AppendLine(CsvHeader);
            obj.AppendLine($"{device2.Id},domain2,deviceGroup2,device2,11,1001,type01,{_lte1.Id},lte1,{_organizationClientApp1.Id},,,False,1,0001-01-01,");
            obj.AppendLine($"{device1.Id},domain1,deviceGroup1,device1,1,1001,type01,{_lte1.Id},lte1,{_organizationClientApp1.Id},,,False,0,0001-01-01,");

            var (response, _, json) = Utils.Post(_client, $"{Url}", Utils.CreateFormContent(obj.ToString(), "csv"), "user1", "user1", 1, "domain1"); // ユーザー管理者
            Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode);
            Assert.NotNull(json["traceId"]);
            Assert.Equal("forbidden", (string)json["errors"]["Role"][0]);
            //Assert.Equal("", device1.Name);
        }
