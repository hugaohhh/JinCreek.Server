    ...
    public class SimDeviceAuthenticationControllerValidateTests : WebApplicationBase
    {
        ...
        class TestDataClass : IEnumerable<object[]>
        {
            ...
            public TestDataClass()
            {
                ...
                //_testData.Add(new object[] { new MultiFactorAuthenticationRequest { Account = "AccountUser1" }, HttpStatusCode.BadRequest, ValidateAssertType.AUTHID_REQUIRED });
                //_testData.Add(new object[] { new MultiFactorAuthenticationRequest { AuthId = Guid.Parse("0e4e88ae-c880-11e2-8598-5855cafa776b"),Account = ""}, HttpStatusCode.BadRequest, ValidateAssertType.ACCOUNT_REQUIRED });
        ...
        public void Validation(SimDeviceAuthenticationRequest request, HttpStatusCode statusCode, ValidateAssertType type)
        {
            ...
            switch (type)
            {
                ...
                case ValidateAssertType.ACCOUNT_REQUIRED: Assert.Equal("account_required", result["errors"]["Account"].First().ToString()); break;
                case ValidateAssertType.AUTHID_REQUIRED: Assert.Equal("authId_required", result["errors"]["AuthId"].First().ToString()); break;
            }
        }
    }
