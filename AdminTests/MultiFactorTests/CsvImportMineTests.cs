...
namespace JinCreek.Server.AdminTests.MultiFactorTests
{
    ...
    public class CsvImportMineTests : IClassFixture<CustomWebApplicationFactoryWithMariaDb<JinCreek.Server.Admin.Startup>>
    {
        ...
        private const string Url = "/api/multi-factors/mine/csv";
        ...
        public void Case16()
        {
            ...
            _context.AddRange(multiFactor2, multiFactor3);
            _context.SaveChanges();
            ...
            //更新データチェック
            Assert.Equal(multiFactor2.Id, list[0]["id"]);
            Assert.Equal(multiFactor2.SimAndDeviceId, list[0]["simAndDeviceId"]);
            Assert.Equal(multiFactor2.EndUserId, list[0]["endUserId"]);
            ...
            //削除データチェック
            Assert.Empty(_context.MultiFactor.Where(s => s.Id == Guid.Parse(list[1]["id"].ToString())));
