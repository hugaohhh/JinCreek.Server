...
namespace JinCreek.Server.AdminTests.SimAndDevicesTests
{
    ...
    public class DeleteMineTests : IClassFixture<CustomWebApplicationFactory<JinCreek.Server.Admin.Startup>>
    {
        ...
        public DeleteMineTests(CustomWebApplicationFactory<JinCreek.Server.Admin.Startup> factory)
        {
            ...
            _context.Add(_simDevice2 = new SimAndDevice() //他組織
            {
                Sim = _sim2,
                Device = _device1,
                IsolatedNw2Ip = "127.0.0.1/18",
        ...
        public void Case02()
        {
            var (response, _, json) = Utils.Delete(_client, $"{Url}/aaa", "user1", "user1", 1, "domain01"); // ユーザー管理者
        ...
        public void Case04()
        {
            var (response, _, json) = Utils.Delete(_client, $"{Url}/{_simDevice2.Id}", "user1", "user1", 1, "domain01"); // ユーザー管理者
            Assert.Equal(HttpStatusCode.Forbidden, response.StatusCode);
            Assert.NotNull(json["traceId"]);
        ...
        public void Case05()
        {
            var (response, _, json) = Utils.Delete(_client, $"{Url}/{_simDevice1.Id}", "user1", "user1", 1, "domain01"); // ユーザー管理者
