...
namespace JinCreek.Server.AdminTests.DeviceTests
{
    ...
    public class ListingTests : IClassFixture<CustomWebApplicationFactory<Admin.Startup>>
    {
        ...
        public ListingTests(CustomWebApplicationFactory<Admin.Startup> factory)
        {
        ...
        public void Case14()
        {
            ...
            _context.SaveChanges();
            var (response,_,json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=1&pageSize=2&startDateTo=2020-02-08&sortBy=StartDate&orderBy=asc", "user0","user0");//    スーパー管理者
            var list = (JArray)json["results"];
        ...
        public void Case15()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=2&pageSize=2&endDateFrom=2020-02-06&sortBy=StartDate&orderBy=desc", "user0","user0");
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Equal(2, list.Count);
        ...
        public void Case16()
        {
            ...
            var (response, _, json) = Utils.Get(_client, Utils.AddQueryString(Url, query), "user0", "user0");
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Equal(20, list.Count);
            Assert.Equal(42, (int)json["count"]);
