...
namespace JinCreek.Server.AdminTests.MultiFactorTests
{
    ...
    public class ListingMineTests : IClassFixture<CustomWebApplicationFactory<JinCreek.Server.Admin.Startup>>
    {
        ...
        public void Case01()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
        ...
        public void Case02()
        {
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=0", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
        ...
        public void Case03()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=30", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Empty(list);
        }
        ...
        public void Case04()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=1&Msisdn=msisdn&sortBy=Msisdn&orderBy=asc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        public void Case05()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=2&pageSize=2&DeviceName=device&sortBy=Msisdn&orderBy=desc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.NotEmpty(list);
            Assert.Equal(2, list.Count);
        ...
        public void Case06()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=3&pageSize=3&UserAccountName=user&sortBy=DeviceName&orderBy=asc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.NotEmpty(list);
        ...
        public void Case07()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=2&startDateFrom=2020-02-06&sortBy=DeviceName&orderBy=desc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        public void Case08()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=1&pageSize=3&startDateTo=2020-02-08&sortBy=UserAccountName&orderBy=asc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.True(3 > list.Count); //'指定件数未満
            Assert.Equal(2, (int)json["count"]);
        ...
        public void Case09()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=2&pageSize=2&endDateFrom=2021-02-06&sortBy=UserAccountName&orderBy=desc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Equal(2, list.Count);
            Assert.Equal(6, (int)json["count"]);
        ...
        public void Case10()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=3&endDateTo=2021-02-08&sortBy=ClosedNwIp&orderBy=asc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        ...
        public void Case11()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=2&pageSize=2&IsAuthenticationDone=True&sortBy=ClosedNwIp&orderBy=desc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Equal(2, list.Count);
            Assert.Equal(6, (int)json["count"]);
        ...
        public void Case12()
        {
            ...
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=1&pageSize=2&Msisdn=msisdn&DeviceName=device&startDateFrom=2019-12-29&startDateTo=2020-02-27&endDateFrom=2020-12-29&endDateTo=2021-02-27&IsAuthenticationDone=True&sortBy=startDate&orderBy=asc", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Equal(2, list.Count);
            Assert.Equal(6, (int)json["count"]);
        ...
        public void Case16()
        {
            SetUpData();
            _context.SaveChanges();
            var (response, _, json) = Utils.Get(_client, $"{Url}/?OrganizationCode={_org1.Code}&page=1&Msisdn=msisdn&DeviceName=device&startDateFrom=2020-02-06&startDateTo=2020-02-08&endDateFrom=2020-02-08&endDateTo=2020-02-10&IsAuthenticationDone=True", "user1", "user1", 1, _domain1.Name);// ユーザー管理者
            var list = (JArray)json["results"];
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Empty(list);
        }
