...
namespace JinCreek.Server.AdminTests.AuthenticationLog
{
    ...
    public class ListingTests : IClassFixture<CustomWebApplicationFactoryWithMariaDb<JinCreek.Server.Admin.Startup>>
    {
        private readonly HttpClient _client;
        private readonly MainDbContext _context;

        private const string Url = "/api/authentication-logs";
        ...
        public ListingTests(CustomWebApplicationFactoryWithMariaDb<JinCreek.Server.Admin.Startup> factory)
        {
        ...
        public void Case02()
        {
            ...
            var (response, body, json) = Utils.Get(_client, Utils.AddQueryString(Url, query), "user0", "user0"); // スーパー管理者
        ...
        /// <summary>
        /// フィルター：通信時刻From、ページ：1、ページサイズ：2、ソート：通信時刻昇順
        /// </summary>
        [Fact]
        public void Case08()
        {
            ...
            Assert.Equal("2020-01-01T02:00:00", (string)list[1]["time"]);// 通信時刻昇順
        }
        ...
        {
            _context.Add(new SimAndDeviceAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T00:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 }); // 範囲外
            for (var i = 0; i < 20; i++) _context.Add(new SimAndDeviceAuthenticationFailureLog { Time = DateTime.Parse("2020-01-01T03:00:00"), Sim = _sim1B });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T01:00:00"), Sim = _sim1B, MultiFactor = _multiFactor1 });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T02:00:00"), Sim = _sim1B, MultiFactor = _multiFactor1 });
            _context.Add(new SimAndDeviceAuthenticationFailureLog { Time = DateTime.Parse("2020-01-01T03:00:00"), Sim = _sim2 }); // 他組織
        ...
        /// <summary>
        /// フィルター：端末、ページ：1、ページサイズ：2、ソート：通信時刻降順
        /// </summary>
        [Fact]
        public void Case10()
        {
            ...
            _context.Add(new DeauthenticationLog { Time = DateTime.Parse("2020-01-01T07:00:00"), Sim = _sim1A, MultiFactor = _multiFactor2 });
            _context.Add(new DeauthenticationLog { Time = DateTime.Parse("2020-01-01T08:00:00"), Sim = _sim1A, MultiFactor = _multiFactor2 });
            _context.Add(new DeauthenticationLog { Time = DateTime.Parse("2020-01-01T09:00:00"), Sim = _sim1A, MultiFactor = _multiFactor2 });
        ...
        /// <summary>
        /// フィルター：内容=SIM＆端末認証、ページ：1、ページサイズ：不在、ソート：通信時刻降順
        /// </summary>
        [Fact]
        public void Case12()
        {
            ...
            Assert.True(20 > list.Count); // 'デフォルト件数以下
            Assert.Equal("2020-01-01T02:00:00", (string)list[0]["time"]);
            Assert.Equal("2020-01-01T01:00:00", (string)list[1]["time"]); // 通信時刻降順
        }

        /// <summary>
        /// フィルター：内容=SIM＆端末認証失敗、ページ：2、ページサイズ：2、ソート：通信時刻降順
        /// </summary>
        [Fact]
        public void Case13()
        {
        ...
        /// <summary>
        /// フィルター：内容=多要素認証、ページ：最終、ページサイズ：2、ソート：通信時刻降順
        /// </summary>
        [Fact]
        public void Case14()
        {
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T00:00:00"), Sim = _sim1A, MultiFactor = _multiFactor1 });
            ...
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T04:00:00"), Sim = _sim1A, MultiFactor = _multiFactor1 });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T05:00:00"), Sim = _sim1A, MultiFactor = _multiFactor1 });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T06:00:00"), Sim = _sim1A, MultiFactor = _multiFactor1 });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T07:00:00"), Sim = _sim1A, MultiFactor = _multiFactor1 });
            _context.Add(new SimAndDeviceAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T00:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 }); // 範囲外
            _context.Add(new SimAndDeviceAuthenticationFailureLog { Time = DateTime.Parse("2020-01-01T01:00:00"), Sim = _sim1A }); // 範囲外
        ...
        /// <summary>
        /// フィルター：内容=多要素認証失敗、ページ：2、ページサイズ：不在、ソート：通信時刻降順
        /// </summary>
        [Fact]
        public void Case15()
        {
            for (var i = 0; i < 20; i++) _context.Add(new MultiFactorAuthenticationFailureLog { Time = DateTime.Parse("2020-01-01T00:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 });
            ...
            _context.Add(new MultiFactorAuthenticationFailureLog { Time = DateTime.Parse("2020-01-01T02:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 });
            for (var i = 0; i < 20; i++) _context.Add(new MultiFactorAuthenticationFailureLog { Time = DateTime.Parse("2020-01-01T03:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 });
            _context.Add(new SimAndDeviceAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T00:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 }); // 範囲外
        ...
        /// <summary>
        /// フィルター：内容=認証解除、ページ：1、ページサイズ：2、ソート：通信時刻降順
        /// </summary>
        [Fact]
        public void Case16()
        {
        ...
        /// <summary>
        /// フィルター：通信時刻・SIM・端末・ユーザ・内容=多要素認証、ページ：2、ページサイズ：2、ソート：通信時刻降順
        /// </summary>
        [Fact]
        public void Case17()
        {
            ...
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T01:00:00"), Sim = _sim1B, MultiFactor = _multiFactor2 });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T02:00:00"), Sim = _sim1B, MultiFactor = _multiFactor2 });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T03:00:00"), Sim = _sim1B, MultiFactor = _multiFactor2 });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T04:00:00"), Sim = _sim1B, MultiFactor = _multiFactor2 });
            _context.Add(new MultiFactorAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T05:00:00"), Sim = _sim1B, MultiFactor = _multiFactor2 });
        ...
        /// <summary>
        /// フィルター：通信時刻To・内容=SIM＆端末認証、ページ：1、ページサイズ：不在、ソート：通信時刻降順
        /// </summary>
        [Fact]
        public void Case19()
        {
            _context.Add(new SimAndDeviceAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T01:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 });
            _context.Add(new SimAndDeviceAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T02:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 });
            _context.Add(new SimAndDeviceAuthenticationSuccessLog { Time = DateTime.Parse("2020-01-01T03:00:00"), Sim = _sim1A, SimAndDevice = _simDevice1 }); // 対象外
            _context.SaveChanges();
